...
func (node BNode) getPtr(idx uint16) uint64 {
	errors.Assert(idx < node.nkeys(), "idx < node.nkeys()")
	pos := HEADER + 8*idx
...
func offsetPos(node BNode, idx uint16) uint16 {
	errors.Assert(1 <= idx && idx <= node.nkeys(), "1 <= idx && idx <= node.nkeys()")
...
func nodeSplit3(old BNode) (uint16, [3]BNode) {
	...
	errors.Assert(leftleft.nbytes() <= BTREE_PAGE_SIZE, "leftleft.nbytes() <= BTREE_PAGE_SIZE")
